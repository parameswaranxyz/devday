...
const EventComponent = ({ dom, event$ }: Sources): Sinks => {
  ...
  const navigateTo$ =
    viewDetailsClick$.compose(sampleCombine(event$))
      .map(([_, { meetup_event_id, meetup_urlname }]) => `https://meetup.com/${meetup_urlname}/events/${meetup_event_id}`);
  const eventRegistration = EventRegistration({ dom });
  const vtree$ =
    xs.combine(event$, eventRegistration.dom)
      .map(([{ title, event_time: { start_time }, abstract, venue: { city }, image_url, url, meetup_event_id, meetup_urlname }, eventRegistration]) =>
        article('.event', { style: fadeInOutStyle, key: url }, [
          div('.media', { style: { delayed: { 'background-image': `url("${image_url}")` } } }, [
            div('.registrations', [eventRegistration]),
